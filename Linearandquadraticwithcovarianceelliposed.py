...
X_shared_covariance,y_shared_covarince = make_data(
    n_samples=300,
    n_features=2,
    cov_class_1=covariance,
    cov_class_2=covariance,
...
def plot_ellipse(mean, cov, color, ax):
    ...
    angle = 180 * angle / np.pi # convert to degress
    # filled Gaussion at 2 standard deviation
...
def plot_result(estimator, X, y , ax):
    ...
    if isinstance(estimator, LinearDiscriminantAnalysis):
        covariance = [estimator.covarince_] * 2
    else:
...
for ax_row, X,y in zip(
    ...
    (y_isotropic_covariance, y_shared_covarince,y_different_covariance)
):
    lda.fit(X,y)
...
axs[1,0].set_ylabel("Data with fixed covarince")
